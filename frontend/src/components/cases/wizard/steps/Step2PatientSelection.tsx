    ...
    <div className="p-6 max-w-2xl">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-400 mt-1">
          Select an existing patient or create a new patient record before starting genomic analysis.
        </p>
      </div>

      <div className="space-y-4">
        {patient ? (
          <div className="p-3 bg-lime-200 border border-lime-500">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-200 mb-1.5">
              ...
              <span className="ml-1.5"><Tooltip text="Patients can have multiple clinical cases across diagnosis, treatment response, and relapse monitoring." /></span>
            </label>
            ...
            {showDropdown && (
              <div
                ...
                id="patient-listbox"
                role="listbox"
                className="absolute z-10 left-0 right-0 mt-1 border border-gray-800 bg-gray-900 shadow-lg max-h-60 overflow-y-auto"
              >
                {isLoading ? (
                  <div className="p-4 text-sm text-gray-200 text-center">Loading...</div>
                ) : isSearching ? (
                  <div className="p-4 text-sm text-gray-200 text-center">Searching...</div>
                ) : patients.length === 0 ? (
                  ...
                  patients.map((p, index) => (
                    <button
                      ...
                      className={`w-full text-left px-4 py-3 transition-colors border-b border-gray-800 last:border-b-0 text-gray-200 ${
                        highlightedIndex === index ? 'bg-lime-200 text-gray-900' : 'hover:bg-gray-900'
                      }`}
                    >
                      <p className="text-sm font-medium">{p.external_patient_id}</p>
                      <p className="text-xs">
