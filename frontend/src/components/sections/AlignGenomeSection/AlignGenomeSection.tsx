...
const services: ServiceConnection[] = [
  { name: "Neo4j", icon: Database, status: "connected", href:"http://localhost:7474/"},
  { name: "PostgreSQL", icon: HardDrive, status: "connected", href:"http://localhost:5050/"},
  { name: "MinIO", icon: FolderOpen, status: "connected", href:"http://localhost:9001/"},
  { name: "API", icon: Cpu, status: "connected", href:"http://localhost:8000/"},
];
...
export function AlignGenomeSection() {
  ...
  const [readLengthOptions, setReadLengthOptions] = useState<number[]>([
    75, 100, 150, 250, 300,
  ]);
      ...
      try {
        ...
        if (result.data) {
          ...
          if (!pipelineConfig.referenceGenome && result.data.length > 0) {
            const firstActive =
              result.data.find((g: GenomeReference) => g.is_active) ||
              result.data[0];
            setPipelineConfig((prev) => ({
              ...prev,
              referenceGenome: firstActive.key,
            }));
          }
        }
      } catch (error) {
    ...
    const loadPipelineSettings = async () => {
      try {
        ...
        if (response.ok) {
          ...
          const optionsSetting = settings.find(
            (s: any) => s.setting_key === "read_length_options",
          );
          const defaultSetting = settings.find(
            (s: any) => s.setting_key === "default_read_length",
          );

          if (optionsSetting) {
            // Parse comma-separated string to array of numbers
            const options = optionsSetting.setting_value
              .split(",")
  ...
  const handleIndexGenome = async () => {
    ...
    setJobId(null);
    setIndexLogs([
      "🚀 Starting genome indexing pipeline...",
    ...
    try {
      ...
      const timeoutId = setTimeout(() => {
        ...
        setIndexLogs((prev) => [
          ...prev,
          "⏱️ Initial connection timeout - server is processing...",
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `🖧 Failed to start indexing: ${response.status} - ${errorText}`,
        );
      }
      ...
      const overallTimeout = setTimeout(
        () => {
          ...
          setIndexLogs((prev) => [
            ...prev,
            "⏱️ Process timeout - indexing taking too long",
          ]);
        },
        30 * 60 * 1000,
      ); // 30 minutes
      ...
      if (error instanceof DOMException && error.name === "AbortError") {
        setIndexLogs((prev) => [
          ...prev,
          "⏱️ Request was cancelled or timed out",
        ]);
      } else {
      ...
      refreshIndexedStatus().catch((err) =>
        console.error("Failed to refresh indexed status:", err),
      );
    }
  };
  ...
  const handleDeleteIndex = async () => {
    ...
    if (
      !confirm(
        `Are you sure you want to delete the index for ${genomeId}? This will remove all index files.`,
      )
    ) {
      return;
    }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <Area>
          <div className="flex items-center gap-3 mb-4">
            ...
            <h3 className="text-lg font-semibold">Genome Indexing Pipeline</h3>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              <div className="flex items-center gap-2 mb-2">
                <label className="text-sm font-medium">Reference Genome</label>
                {indexedStatus[pipelineConfig.referenceGenome] && (
                  <div className="flex items-center gap-1 text-zinc-800 bg-lime-300 text-xs px-2 py-0.5 rounded">
            ...
            <div>
              ...
              <Select
                ...
                onChange={(e) =>
                  setSelectedReadLength(parseInt(e.target.value, 10))
                }
              >
            ...
            <div className="text-sm text-muted-foreground bg-muted p-3">
              ...
              <div className="space-y-2">
                ...
                {(currentStage || jobId) && (
                  <div className="flex items-center gap-4 text-xs">
                    {jobId && (
                      <div className="bg-zinc-800 text-lime-200 px-2 py-1">
                        Job ID: {jobId}
                      </div>
                    )}
                    {currentStage && (
                      <div className="bg-zinc-800 text-lime-300 px-2 py-1">
                        Stage:{" "}
                        {{
                          initializing: "Initializing",
                          downloading: "Downloading genome",
          ...
          <div className="space-y-2">
            ...
            {indexedStatus[pipelineConfig.referenceGenome] && (
              <div className="flex items-center gap-2 text-sm">
                <div className="flex items-center gap-1 text-zinc-800 bg-lime-300 px-2 py-1 rounded">
                  <CheckCircle className="h-3 w-3" />
        ...
        <Area>
          ...
          <div className="space-y-3">
            {services.map(({ name, icon: Icon, status, href }) => (
              <div
                key={name}
